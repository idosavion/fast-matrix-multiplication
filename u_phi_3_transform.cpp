...
static inline int transformed_block_size(int block_size, int steps_left) {
	int base_block_size = block_size / (pow(12, steps_left));
	int power = pow(14, steps_left-1);
...
#endif
	for (int k = 0; k < block_size; k++) {
		S8[k] = -matrixA[(4 * block_size) + k] + matrixA[(8 * block_size) + k];
	}
}
...
#endif
	for (int k = 0; k < block_size; k++) {
		S9[k] = matrixA[(4 * block_size) + k] + matrixA[(10 * block_size) + k];
	}
}
...
#endif
	for (int k = 0; k < block_size; k++) {
		S14[k] = matrixA[(11 * block_size) + k];
	}
}


void u_phi_3_transform(double* original_a, double* transformed_a, int size, int steps_left, double* work) {
	int block_size = size / 12;
	//int next_block_size = block_size / 12;
	...
	u_phi_3_transform(S, transformed_a, block_size, steps_left - 1, nextWork);

	S2_sum(original_a, S, block_size);
	u_phi_3_transform(S, &transformed_a[trans_block_size], block_size, steps_left - 1, nextWork);

	S3_sum(original_a, S, block_size);
	u_phi_3_transform(S, &transformed_a[2 * trans_block_size], block_size, steps_left - 1, nextWork);

	S4_sum(original_a, S, block_size);
	u_phi_3_transform(S, &transformed_a[3 * trans_block_size], block_size, steps_left - 1, nextWork);

	S5_sum(original_a, S, block_size);
	u_phi_3_transform(S, &transformed_a[4 * trans_block_size], block_size, steps_left - 1, nextWork);

	S6_sum(original_a, S, block_size);
	u_phi_3_transform(S, &transformed_a[5 * trans_block_size], block_size, steps_left - 1, nextWork);

	S7_sum(original_a, S, block_size);
	u_phi_3_transform(S, &transformed_a[6 * trans_block_size], block_size, steps_left - 1, nextWork);

	S8_sum(original_a, S, block_size);
	u_phi_3_transform(S, &transformed_a[7 * trans_block_size], block_size, steps_left - 1, nextWork);

	S9_sum(original_a, S, block_size);
	u_phi_3_transform(S, &transformed_a[8 * trans_block_size], block_size, steps_left - 1, nextWork);

	S10_sum(original_a, S, block_size);
	u_phi_3_transform(S, &transformed_a[9 * trans_block_size], block_size, steps_left - 1, nextWork);

	S11_sum(original_a, S, block_size);
	u_phi_3_transform(S, &transformed_a[10 * trans_block_size], block_size, steps_left - 1, nextWork);

	S12_sum(original_a, S, block_size);
	u_phi_3_transform(S, &transformed_a[11 * trans_block_size], block_size, steps_left - 1, nextWork);

	S13_sum(original_a, S, block_size);
	u_phi_3_transform(S, &transformed_a[12 * trans_block_size], block_size, steps_left - 1, nextWork);

	S14_sum(original_a, S, block_size);
	u_phi_3_transform(S, &transformed_a[13 * trans_block_size], block_size, steps_left - 1, nextWork);
}