...
static void S2_sum(const double* matrixA, double* S2, int block_size){
    ...
    #endif
    for (int k = 0; k < block_size ; k++){
        S2[k] = - matrixA[k] + matrixA[( 2 * block_size ) + k];
    }
}
...
static void S5_sum(const double* matrixA, double* S5, int block_size){
    ...
    #endif
    for (int k = 0; k < block_size ; k++){
        S5[k] = matrixA[k] + matrixA[( 5 * block_size ) + k];
    }
}
...
static void S6_sum(const double* matrixA, double* S6, int block_size){
    ...
    #endif
    for (int k = 0; k < block_size ; k++){
        S6[k] = - matrixA[k] + matrixA[( 6 * block_size ) + k];
    }
}
...
static void S8_sum(const double* matrixA, double* S8, int block_size){
    ...
    #endif
    for (int k = 0; k < block_size ; k++){
        S8[k] = matrixA[k] + matrixA[( 8 * block_size ) + k];
    }
}
...
static void S10_sum(const double* matrixA, double* S10, int block_size){
    ...
    #endif
    for (int k = 0; k < block_size ; k++){
        S10[k] = matrixA[( 10 * block_size ) + k];
    }
}
void w_tau_2_transform(double* original_a, double* transformed_a, int size, int steps_left, double* work){
    ...
    w_tau_2_transform(S, transformed_a, block_size, steps_left - 1, nextWork);    S2_sum(original_a, S, block_size);
    w_tau_2_transform(S, &transformed_a[1 * trans_block_size], block_size, steps_left - 1, nextWork);

    S3_sum(original_a, S, block_size);
    w_tau_2_transform(S, &transformed_a[2 * trans_block_size], block_size, steps_left - 1, nextWork);

    S4_sum(original_a, S, block_size);
    w_tau_2_transform(S, &transformed_a[3 * trans_block_size], block_size, steps_left - 1, nextWork);

    S5_sum(original_a, S, block_size);
    w_tau_2_transform(S, &transformed_a[4 * trans_block_size], block_size, steps_left - 1, nextWork);

    S6_sum(original_a, S, block_size);
    w_tau_2_transform(S, &transformed_a[5 * trans_block_size], block_size, steps_left - 1, nextWork);

    S7_sum(original_a, S, block_size);
    w_tau_2_transform(S, &transformed_a[6 * trans_block_size], block_size, steps_left - 1, nextWork);

    S8_sum(original_a, S, block_size);
    w_tau_2_transform(S, &transformed_a[7 * trans_block_size], block_size, steps_left - 1, nextWork);

    S9_sum(original_a, S, block_size);
    w_tau_2_transform(S, &transformed_a[8 * trans_block_size], block_size, steps_left - 1, nextWork);

    S10_sum(original_a, S, block_size);
    w_tau_2_transform(S, &transformed_a[9 * trans_block_size], block_size, steps_left - 1, nextWork);
