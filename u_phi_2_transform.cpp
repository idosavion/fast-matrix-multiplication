...
static inline int transformed_block_size(int block_size, int steps_left) {
	int base_block_size = block_size / (pow(10, steps_left));
	int pow_of_twelve = pow(12, steps_left-1);

	return pow_of_twelve * base_block_size;
}
...
#endif
	for (int k = 0; k < block_size; k++) {
		S2[k] = -matrixA[(1 * block_size) + k] - matrixA[(3 * block_size) + k] - matrixA[(6 * block_size) + k] - matrixA[(9 * block_size) + k];
	}
}
...
#endif
	for (int k = 0; k < block_size; k++) {
		S5[k] = -matrixA[(5 * block_size) + k] + matrixA[(6 * block_size) + k] - matrixA[(7 * block_size) + k] + matrixA[(9 * block_size) + k];
	}
}
...
void u_phi_2_transform(double* original_a, double* transformed_a, int size, int steps_left, double* work) {
	int block_size = size / 10;
	//int next_block_size = block_size / 10;
	...
	u_phi_2_transform(S, transformed_a, block_size, steps_left - 1, nextWork);

	S2_sum(original_a, S, block_size);
	u_phi_2_transform(S, &transformed_a[trans_block_size], block_size, steps_left - 1, nextWork);

	S3_sum(original_a, S, block_size);
	u_phi_2_transform(S, &transformed_a[2 * trans_block_size], block_size, steps_left - 1, nextWork);

	S4_sum(original_a, S, block_size);
	u_phi_2_transform(S, &transformed_a[3 * trans_block_size], block_size, steps_left - 1, nextWork);

	S5_sum(original_a, S, block_size);
	u_phi_2_transform(S, &transformed_a[4 * trans_block_size], block_size, steps_left - 1, nextWork);

	S6_sum(original_a, S, block_size);
	u_phi_2_transform(S, &transformed_a[5 * trans_block_size], block_size, steps_left - 1, nextWork);

	S7_sum(original_a, S, block_size);
	u_phi_2_transform(S, &transformed_a[6 * trans_block_size], block_size, steps_left - 1, nextWork);

	S8_sum(original_a, S, block_size);
	u_phi_2_transform(S, &transformed_a[7 * trans_block_size], block_size, steps_left - 1, nextWork);

	S9_sum(original_a, S, block_size);
	u_phi_2_transform(S, &transformed_a[8 * trans_block_size], block_size, steps_left - 1, nextWork);

	S10_sum(original_a, S, block_size);
	u_phi_2_transform(S, &transformed_a[9 * trans_block_size], block_size, steps_left - 1, nextWork);

	S11_sum(original_a, S, block_size);
	u_phi_2_transform(S, &transformed_a[10 * trans_block_size], block_size, steps_left - 1, nextWork);

	S12_sum(original_a, S, block_size);
	u_phi_2_transform(S, &transformed_a[11 * trans_block_size], block_size, steps_left - 1, nextWork);
}