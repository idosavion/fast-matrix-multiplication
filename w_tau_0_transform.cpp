...
static void S2_sum(const double* matrixA, double* S2, int block_size){
    ...
    #endif
    for (int k = 0; k < block_size ; k++){
        S2[k] = - matrixA[k]- matrixA[( 1 * block_size ) + k];
    }
}
...
static void S4_sum(const double* matrixA, double* S4, int block_size){
    ...
    #endif
    for (int k = 0; k < block_size ; k++){
        S4[k] = - matrixA[( 3 * block_size ) + k] + matrixA[( 4 * block_size ) + k] + matrixA[( 6 * block_size ) + k];
    }
}
...
static void S5_sum(const double* matrixA, double* S5, int block_size){
    ...
    #endif
    for (int k = 0; k < block_size ; k++){
        S5[k] = - matrixA[( 1 * block_size ) + k]- matrixA[( 4 * block_size ) + k];
    }
}
...
static void S6_sum(const double* matrixA, double* S6, int block_size){
    ...
    #endif
    for (int k = 0; k < block_size ; k++){
        S6[k] = matrixA[( 5 * block_size ) + k] + matrixA[( 7 * block_size ) + k]- matrixA[( 8 * block_size ) + k];
    }
}
...
static void S7_sum(const double* matrixA, double* S7, int block_size){
    ...
    #endif
    for (int k = 0; k < block_size ; k++){
        S7[k] = - matrixA[( 4 * block_size ) + k]- matrixA[( 6 * block_size ) + k];
    }
}
...
static void S8_sum(const double* matrixA, double* S8, int block_size){
    ...
    #endif
    for (int k = 0; k < block_size ; k++){
        S8[k] = matrixA[( 2 * block_size ) + k] + matrixA[( 4 * block_size ) + k] + matrixA[( 6 * block_size ) + k]- matrixA[( 7 * block_size ) + k];
    }
}
...
static void S9_sum(const double* matrixA, double* S9, int block_size){
    ...
    #endif
    for (int k = 0; k < block_size ; k++){
        S9[k] = matrixA[( 2 * block_size ) + k]- matrixA[( 7 * block_size ) + k] + matrixA[( 8 * block_size ) + k];
    }
}
void w_tau_0_transform(double* original_a, double* transformed_a, int size, int steps_left, double* work){
    ...
    w_tau_0_transform(S, transformed_a, block_size, steps_left - 1, nextWork);    S2_sum(original_a, S, block_size);
    w_tau_0_transform(S, &transformed_a[1 * trans_block_size], block_size, steps_left - 1, nextWork);

    S3_sum(original_a, S, block_size);
    w_tau_0_transform(S, &transformed_a[2 * trans_block_size], block_size, steps_left - 1, nextWork);

    S4_sum(original_a, S, block_size);
    w_tau_0_transform(S, &transformed_a[3 * trans_block_size], block_size, steps_left - 1, nextWork);

    S5_sum(original_a, S, block_size);
    w_tau_0_transform(S, &transformed_a[4 * trans_block_size], block_size, steps_left - 1, nextWork);

    S6_sum(original_a, S, block_size);
    w_tau_0_transform(S, &transformed_a[5 * trans_block_size], block_size, steps_left - 1, nextWork);

    S7_sum(original_a, S, block_size);
    w_tau_0_transform(S, &transformed_a[6 * trans_block_size], block_size, steps_left - 1, nextWork);

    S8_sum(original_a, S, block_size);
    w_tau_0_transform(S, &transformed_a[7 * trans_block_size], block_size, steps_left - 1, nextWork);

    S9_sum(original_a, S, block_size);
    w_tau_0_transform(S, &transformed_a[8 * trans_block_size], block_size, steps_left - 1, nextWork);
