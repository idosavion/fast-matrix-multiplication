...
static void S3_sum(const double* matrixA, double* S3, int block_size){
    ...
    #endif
    for (int k = 0; k < block_size ; k++){
        S3[k] = - matrixA[( 4 * block_size ) + k]- matrixA[( 6 * block_size ) + k];
    }
}
...
static void S4_sum(const double* matrixA, double* S4, int block_size){
    ...
    #endif
    for (int k = 0; k < block_size ; k++){
        S4[k] = matrixA[k] + matrixA[( 5 * block_size ) + k];
    }
}
...
static void S8_sum(const double* matrixA, double* S8, int block_size){
    ...
    #endif
    for (int k = 0; k < block_size ; k++){
        S8[k] = - matrixA[( 1 * block_size ) + k]- matrixA[( 5 * block_size ) + k]- matrixA[( 7 * block_size ) + k] + matrixA[( 8 * block_size ) + k];
    }
}
...
static void S10_sum(const double* matrixA, double* S10, int block_size){
    ...
    #endif
    for (int k = 0; k < block_size ; k++){
        S10[k] = - matrixA[( 2 * block_size ) + k];
    }
}
void v_psi_1_transform(double* original_a, double* transformed_a, int size, int steps_left, double* work){
    ...
    v_psi_1_transform(S, transformed_a, block_size, steps_left - 1, nextWork);

    S2_sum(original_a, S, block_size);
    v_psi_1_transform(S, &transformed_a[1 * trans_block_size], block_size, steps_left - 1, nextWork);

    S3_sum(original_a, S, block_size);
    v_psi_1_transform(S, &transformed_a[2 * trans_block_size], block_size, steps_left - 1, nextWork);

    S4_sum(original_a, S, block_size);
    v_psi_1_transform(S, &transformed_a[3 * trans_block_size], block_size, steps_left - 1, nextWork);

    S5_sum(original_a, S, block_size);
    v_psi_1_transform(S, &transformed_a[4 * trans_block_size], block_size, steps_left - 1, nextWork);

    S6_sum(original_a, S, block_size);
    v_psi_1_transform(S, &transformed_a[5 * trans_block_size], block_size, steps_left - 1, nextWork);

    S7_sum(original_a, S, block_size);
    v_psi_1_transform(S, &transformed_a[6 * trans_block_size], block_size, steps_left - 1, nextWork);

    S8_sum(original_a, S, block_size);
    v_psi_1_transform(S, &transformed_a[7 * trans_block_size], block_size, steps_left - 1, nextWork);

    S9_sum(original_a, S, block_size);
    v_psi_1_transform(S, &transformed_a[8 * trans_block_size], block_size, steps_left - 1, nextWork);

    S10_sum(original_a, S, block_size);
    v_psi_1_transform(S, &transformed_a[9 * trans_block_size], block_size, steps_left - 1, nextWork);
