...
static void S6_sum(const double* matrixA, double* S6, int block_size){
    ...
    #endif
    for (int k = 0; k < block_size ; k++){
        S6[k] = - matrixA[( 1 * block_size ) + k] + matrixA[( 7 * block_size ) + k];
    }
}
...
static void S7_sum(const double* matrixA, double* S7, int block_size){
    ...
    #endif
    for (int k = 0; k < block_size ; k++){
        S7[k] = matrixA[( 2 * block_size ) + k]- matrixA[( 9 * block_size ) + k];
    }
}
...
static void S8_sum(const double* matrixA, double* S8, int block_size){
    ...
    #endif
    for (int k = 0; k < block_size ; k++){
        S8[k] = - matrixA[( 7 * block_size ) + k]- matrixA[( 9 * block_size ) + k];
    }
}
...
static void S9_sum(const double* matrixA, double* S9, int block_size){
    ...
    #endif
    for (int k = 0; k < block_size ; k++){
        S9[k] = - matrixA[k]- matrixA[( 9 * block_size ) + k];
    }
}
void w_tau_1_transform(double* original_a, double* transformed_a, int size, int steps_left, double* work){
    ...
    w_tau_1_transform(S, transformed_a, block_size, steps_left - 1, nextWork);    S2_sum(original_a, S, block_size);
    w_tau_1_transform(S, &transformed_a[1 * trans_block_size], block_size, steps_left - 1, nextWork);

    S3_sum(original_a, S, block_size);
    w_tau_1_transform(S, &transformed_a[2 * trans_block_size], block_size, steps_left - 1, nextWork);

    S4_sum(original_a, S, block_size);
    w_tau_1_transform(S, &transformed_a[3 * trans_block_size], block_size, steps_left - 1, nextWork);

    S5_sum(original_a, S, block_size);
    w_tau_1_transform(S, &transformed_a[4 * trans_block_size], block_size, steps_left - 1, nextWork);

    S6_sum(original_a, S, block_size);
    w_tau_1_transform(S, &transformed_a[5 * trans_block_size], block_size, steps_left - 1, nextWork);

    S7_sum(original_a, S, block_size);
    w_tau_1_transform(S, &transformed_a[6 * trans_block_size], block_size, steps_left - 1, nextWork);

    S8_sum(original_a, S, block_size);
    w_tau_1_transform(S, &transformed_a[7 * trans_block_size], block_size, steps_left - 1, nextWork);

    S9_sum(original_a, S, block_size);
    w_tau_1_transform(S, &transformed_a[8 * trans_block_size], block_size, steps_left - 1, nextWork);
