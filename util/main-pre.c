...
#pragma omp parallel for
#endif
		for (int j = 0; j < i / 2; j++) {
			vdAdd(i / 2, &matrixA[j * i], &matrixA[i*(j+i/2) + i/2], &matrixC_strassen[j * (i / 2)]);
			vdSub(i / 2, matrixC_strassen, &matrixA[i * j + i / 2], matrixC_strassen);
			//for (int k = 0; k < i / 2; k++) {
			//	matrixC_strassen[j * (i / 2) + k] = matrixA[j * i + k] + matrixA[(i / 2 + j) * i + (i / 2 + k)] - matrixA[j * i + (i / 2 + k)];
			//}
		}
	...
	if (vml == true) {
		...
		start1 = clock();
		vdAdd(i, matrixC, matrixB, matrixC_strassen);
		vdSub(i, matrixC_strassen, matrixC, matrixC_strassen);
		end = clock();
	...
	if (blockAdd == true) {
		start = clock();
		mkl_domatadd('R', 'N', 'N', i / 2, i / 2, 1, &matrixA[i * i / 2 + i / 2], i, 1, &matrixA[i / 2], i, matrixC_strassen, i / 2);
		mkl_domatadd('R', 'N', 'N', i / 2, i / 2, 1, matrixC_strassen, i/2, -1, matrixA, i, matrixC_strassen, i / 2);
		end = clock();
		msec = ((double)((end - start) * 1000) / CLOCKS_PER_SEC);
