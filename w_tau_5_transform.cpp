...
static void S1_sum(const double* matrixA, double* S1, int block_size){
    ...
    for (int k = 0; k < block_size ; k++){
        S1[k] = matrixA[( 1 * block_size ) + k] + matrixA[( 3 * block_size ) + k];
    }
}
...
static void S4_sum(const double* matrixA, double* S4, int block_size){
    ...
    #endif
    for (int k = 0; k < block_size ; k++){
        S4[k] = matrixA[( 1 * block_size ) + k] + matrixA[( 6 * block_size ) + k];
    }
}
...
static void S8_sum(const double* matrixA, double* S8, int block_size){
    ...
    #endif
    for (int k = 0; k < block_size ; k++){
        S8[k] = matrixA[k] + matrixA[( 10 * block_size ) + k];
    }
}
...
static void S10_sum(const double* matrixA, double* S10, int block_size){
    ...
    #endif
    for (int k = 0; k < block_size ; k++){
        S10[k] = - matrixA[( 2 * block_size ) + k] + matrixA[( 12 * block_size ) + k];
    }
}
...
static void S11_sum(const double* matrixA, double* S11, int block_size){
    ...
    #endif
    for (int k = 0; k < block_size ; k++){
        S11[k] = matrixA[k] + matrixA[( 13 * block_size ) + k];
    }
}
...
static void S16_sum(const double* matrixA, double* S16, int block_size){
    ...
    #endif
    for (int k = 0; k < block_size ; k++){
        S16[k] = matrixA[( 2 * block_size ) + k] + matrixA[( 18 * block_size ) + k];
    }
}
...
static void S17_sum(const double* matrixA, double* S17, int block_size){
    ...
    #endif
    for (int k = 0; k < block_size ; k++){
        S17[k] = matrixA[( 19 * block_size ) + k];
    }
}
void w_tau_5_transform(double* original_a, double* transformed_a, int size, int steps_left, double* work){
    ...
    w_tau_5_transform(S, transformed_a, block_size, steps_left - 1, nextWork);    S2_sum(original_a, S, block_size);
    w_tau_5_transform(S, &transformed_a[1 * trans_block_size], block_size, steps_left - 1, nextWork);

    S3_sum(original_a, S, block_size);
    w_tau_5_transform(S, &transformed_a[2 * trans_block_size], block_size, steps_left - 1, nextWork);

    S4_sum(original_a, S, block_size);
    w_tau_5_transform(S, &transformed_a[3 * trans_block_size], block_size, steps_left - 1, nextWork);

    S5_sum(original_a, S, block_size);
    w_tau_5_transform(S, &transformed_a[4 * trans_block_size], block_size, steps_left - 1, nextWork);

    S6_sum(original_a, S, block_size);
    w_tau_5_transform(S, &transformed_a[5 * trans_block_size], block_size, steps_left - 1, nextWork);

    S7_sum(original_a, S, block_size);
    w_tau_5_transform(S, &transformed_a[6 * trans_block_size], block_size, steps_left - 1, nextWork);

    S8_sum(original_a, S, block_size);
    w_tau_5_transform(S, &transformed_a[7 * trans_block_size], block_size, steps_left - 1, nextWork);

    S9_sum(original_a, S, block_size);
    w_tau_5_transform(S, &transformed_a[8 * trans_block_size], block_size, steps_left - 1, nextWork);

    S10_sum(original_a, S, block_size);
    w_tau_5_transform(S, &transformed_a[9 * trans_block_size], block_size, steps_left - 1, nextWork);

    S11_sum(original_a, S, block_size);
    w_tau_5_transform(S, &transformed_a[10 * trans_block_size], block_size, steps_left - 1, nextWork);

    S12_sum(original_a, S, block_size);
    w_tau_5_transform(S, &transformed_a[11 * trans_block_size], block_size, steps_left - 1, nextWork);

    S13_sum(original_a, S, block_size);
    w_tau_5_transform(S, &transformed_a[12 * trans_block_size], block_size, steps_left - 1, nextWork);

    S14_sum(original_a, S, block_size);
    w_tau_5_transform(S, &transformed_a[13 * trans_block_size], block_size, steps_left - 1, nextWork);

    S15_sum(original_a, S, block_size);
    w_tau_5_transform(S, &transformed_a[14 * trans_block_size], block_size, steps_left - 1, nextWork);

    S16_sum(original_a, S, block_size);
    w_tau_5_transform(S, &transformed_a[15 * trans_block_size], block_size, steps_left - 1, nextWork);

    S17_sum(original_a, S, block_size);
    w_tau_5_transform(S, &transformed_a[16 * trans_block_size], block_size, steps_left - 1, nextWork);
