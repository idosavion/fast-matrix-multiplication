...
static void S2_sum(const double* matrixA, double* S2, int block_size){
    ...
    #endif
    for (int k = 0; k < block_size ; k++){
        S2[k] = matrixA[k] + matrixA[( 12 * block_size ) + k];
    }
}
...
static void S3_sum(const double* matrixA, double* S3, int block_size){
    ...
    #endif
    for (int k = 0; k < block_size ; k++){
        S3[k] = matrixA[( 3 * block_size ) + k]- matrixA[( 14 * block_size ) + k];
    }
}
...
void v_psi_5_transform(double* original_a, double* transformed_a, int size, int steps_left, double* work){
    ...
    v_psi_5_transform(S, transformed_a, block_size, steps_left - 1, nextWork);

    S2_sum(original_a, S, block_size);
    v_psi_5_transform(S, &transformed_a[1 * trans_block_size], block_size, steps_left - 1, nextWork);

    S3_sum(original_a, S, block_size);
    v_psi_5_transform(S, &transformed_a[2 * trans_block_size], block_size, steps_left - 1, nextWork);

    S4_sum(original_a, S, block_size);
    v_psi_5_transform(S, &transformed_a[3 * trans_block_size], block_size, steps_left - 1, nextWork);

    S5_sum(original_a, S, block_size);
    v_psi_5_transform(S, &transformed_a[4 * trans_block_size], block_size, steps_left - 1, nextWork);

    S6_sum(original_a, S, block_size);
    v_psi_5_transform(S, &transformed_a[5 * trans_block_size], block_size, steps_left - 1, nextWork);

    S7_sum(original_a, S, block_size);
    v_psi_5_transform(S, &transformed_a[6 * trans_block_size], block_size, steps_left - 1, nextWork);

    S8_sum(original_a, S, block_size);
    v_psi_5_transform(S, &transformed_a[7 * trans_block_size], block_size, steps_left - 1, nextWork);

    S9_sum(original_a, S, block_size);
    v_psi_5_transform(S, &transformed_a[8 * trans_block_size], block_size, steps_left - 1, nextWork);

    S10_sum(original_a, S, block_size);
    v_psi_5_transform(S, &transformed_a[9 * trans_block_size], block_size, steps_left - 1, nextWork);

    S11_sum(original_a, S, block_size);
    v_psi_5_transform(S, &transformed_a[10 * trans_block_size], block_size, steps_left - 1, nextWork);

    S12_sum(original_a, S, block_size);
    v_psi_5_transform(S, &transformed_a[11 * trans_block_size], block_size, steps_left - 1, nextWork);

    S13_sum(original_a, S, block_size);
    v_psi_5_transform(S, &transformed_a[12 * trans_block_size], block_size, steps_left - 1, nextWork);

    S14_sum(original_a, S, block_size);
    v_psi_5_transform(S, &transformed_a[13 * trans_block_size], block_size, steps_left - 1, nextWork);

    S15_sum(original_a, S, block_size);
    v_psi_5_transform(S, &transformed_a[14 * trans_block_size], block_size, steps_left - 1, nextWork);

    S16_sum(original_a, S, block_size);
    v_psi_5_transform(S, &transformed_a[15 * trans_block_size], block_size, steps_left - 1, nextWork);

    S17_sum(original_a, S, block_size);
    v_psi_5_transform(S, &transformed_a[16 * trans_block_size], block_size, steps_left - 1, nextWork);

    S18_sum(original_a, S, block_size);
    v_psi_5_transform(S, &transformed_a[17 * trans_block_size], block_size, steps_left - 1, nextWork);

    S19_sum(original_a, S, block_size);
    v_psi_5_transform(S, &transformed_a[18 * trans_block_size], block_size, steps_left - 1, nextWork);

    S20_sum(original_a, S, block_size);
    v_psi_5_transform(S, &transformed_a[19 * trans_block_size], block_size, steps_left - 1, nextWork);
